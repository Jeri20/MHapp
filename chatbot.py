...
# Load DialoGPT (small for faster loading)
tokenizer = AutoTokenizer.from_pretrained("microsoft/DialoGPT-small")
model = AutoModelForCausalLM.from_pretrained("microsoft/DialoGPT-small")

# Store past chat history
chat_history = {}
...
def get_chat_response(user_id, user_input):
    ...
    bot_input_ids = torch.cat([torch.tensor([])] + chat_history[user_id] + [new_input_ids], dim=-1) if chat_history[user_id] else new_input_ids
